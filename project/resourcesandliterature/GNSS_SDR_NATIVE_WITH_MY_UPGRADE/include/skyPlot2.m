function hpol = skyPlot2(varargin)
...
if strcmp(line_style, 'auto')
    ...
    hpol = plot(hAxis, xx', yy', '.-', 'LineWidth',4);
else
...
plot(hAxis, xx(:,end)', yy(:,end)', 'o','MarkerFaceColor','y', 'LineWidth',2, 'MarkerSize', 10);
...
for i = 1:length(prn)
    if(prn(i) ~= 0)
        ...
        % is used.
        text(xx(i, end), yy(i, end), ['  ', int2str(prn(i))], 'color', 'k','FontSize',14);
    end
end
