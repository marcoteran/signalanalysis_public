function plotNavigationRus(navSolutions, settings)
...
if (~isempty(navSolutions))
    ...
    if isnan(settings.truePosition.E) || isnan(settings.truePosition.N) ...
        ...
        refPointLgText = ['средняя позиция\newline широта: ', ...
    ...
    figure(figureNumber);
    clf   (figureNumber);
    set   (figureNumber, 'Name', 'Решение НВО');
...
%% Plot all figures =======================================================
    ...
    plot(handles(1, 1), [(navSolutions.E - refCoord.E)', ...
                         ...
                         (navSolutions.U - refCoord.U)'],'LineWidth',3);
    title (handles(1, 1), 'разброс координат в системе UTM','fontsize',10);   
    legend(handles(1, 1), 'E', 'N', 'U','fontsize',10);   
    xlabel(handles(1, 1), ['Период измерения: ', ...
                                    num2str(settings.navSolPeriod), 'ms'],'fontsize',10);   
    ylabel(handles(1, 1), 'разборос, м','fontsize',10);   
    grid  (handles(1, 1));
    ...
    plot3 (handles(3, 1), navSolutions.E - refCoord.E, ...
                          navSolutions.N - refCoord.N, ... 
                          navSolutions.U - refCoord.U, 'rs', 'LineWidth', 1.5, 'MarkerSize', 10);
    hold  (handles(3, 1), 'on');
    %Plot the reference point
    plot3 (handles(3, 1), 0, 0, 0, 'bs', 'LineWidth', 1.5, 'MarkerSize', 10);
    ...
    legend(handles(3, 1), 'разброс', refPointLgText);
 
    title (handles(3, 1), 'разброс координат в системе UTM (3D)','fontsize',10);
    xlabel(handles(3, 1), 'восток, м','fontsize',10);
    ylabel(handles(3, 1), 'север, м','fontsize',10);
    zlabel(handles(3, 1), 'вверх, м','fontsize',10);
    
    %--- Satellite sky plot -----------------------------------------------
    skyPlot2(handles(3, 2), ...
            navSolutions.channel.az, ...
            navSolutions.channel.el, ...
            navSolutions.channel.PRN(:, 1));
        
    title (handles(3, 2), ['Созвездия спутников  (средный ГФ: ', ...
